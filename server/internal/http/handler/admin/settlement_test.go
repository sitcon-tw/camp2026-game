...
func TestBuildSettlementResponse(t *testing.T) {
	...
	data := settlementData{
		...
		Inventory: []dashboardInventoryStat{{ID: settlementTShirtItemID, Quantity: 7}},
		Front:     &mongomodel.Front{Teams: []mongomodel.FrontTeam{{TeamID: "a", Name: "A", ControlledCells: 10}, {TeamID: "b", Name: "B", ControlledCells: 20}}},
	...
	if got.TShirtCount != 7 {
		t.Fatalf("tshirt count = %d, want 7", got.TShirtCount)
	}
