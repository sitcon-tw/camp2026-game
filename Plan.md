...
| 小石受困 | 小石暫時不能上前線 | cooldown 或受困 10 到 30 分鐘 | 每人同時最多 1 顆；Day 5 前全部歸還 |
...
3. 玩家選一顆小石與一個相鄰節點，下達 `擴張`、`攻擊`、`防守`、`修復`、`偵查` 或 `救援` command。
4. Server 接收 command，進入 pending 狀態，前端立即顯示箭頭、進度條或小石行動中。
5. Server 以固定 tick 推進地圖，彙整同隊與敵隊壓力。
6. 節點 owner、control、defense、pressure、resource、event 變化後，SSE 或 polling 推送 delta。
...
SSE 是即時感的核心，但必須有 polling fallback：

- 首選：`GET /api/fronts/{frontID}/events` 以 SSE 推送 changed cells、leaderboard、event log、tick。
- fallback：前端每 3 到 5 秒 `GET /api/fronts/{frontID}`。
